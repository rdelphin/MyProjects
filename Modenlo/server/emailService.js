...
// Admin email address
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@modenlo.com';
...
try {
    ...
    transporter.verify()
        ...
        .catch((error) => {
            ...
            console.log('   Please check your email credentials (EMAIL_USER and EMAIL_PASS)');
            emailConfigured = false;
        });
} catch (error) {
    console.error('\n❌ Email transporter creation failed:', error.message);
    console.log('📧 Email functionality will be disabled.');
    console.log('   To enable emails, set these environment variables:');
    console.log('   - EMAIL_USER (your email address)');
    console.log('   - EMAIL_PASS (app password for Gmail, or password for other services)');
    console.log('   - ADMIN_EMAIL (email to receive order notifications)\n');
}
...
async function sendCustomerConfirmation(orderData) {
    if (!transporter || !emailConfigured) {
        console.log('⚠️  Email not configured - customer confirmation NOT sent to:', orderData.contact.email);
...
Thank you for choosing Modenlo!
...
The Modenlo Team
    ...
    const mailOptions = {
        from: `"Modenlo" <${EMAIL_CONFIG.auth.user}>`,
        to: contact.email,
        ...
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <p style="margin-top: 30px;">Thank you for choosing Modenlo!</p>
                <p><strong>The Modenlo Team</strong></p>
            </div>
        `
    };
...
async function sendAdminNotification(orderData, orderId) {
    if (!transporter || !emailConfigured) {
        console.log('⚠️  Email not configured - admin notification NOT sent for order:', orderId);
        console.log('   Set EMAIL_USER, EMAIL_PASS, and ADMIN_EMAIL environment variables to enable emails');
        return { success: false, message: 'Email not configured' };
    }
...
Order Date: ${new Date(order.orderDate).toLocaleString()}
    ...
    const mailOptions = {
        from: `"Modenlo System" <${EMAIL_CONFIG.auth.user}>`,
        to: ADMIN_EMAIL,
    ...
    try {
        ...
        console.log('   Message ID:', info.messageId);
        console.log('   Download link sent to:', ADMIN_EMAIL);
        return { success: true, downloadToken, expiresAt, messageId: info.messageId };
    } catch (error) {
